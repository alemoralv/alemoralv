...
for(const [name,width,height] of [['desktop',1440,1000],['phone',390,844]]) {
 ...
 await page.goto('http://127.0.0.1:4519/',{waitUntil:'domcontentloaded'});await page.evaluate(()=>document.fonts.ready);await page.waitForTimeout(500);
 for(const section of ['home','about','projects','notes','academic-background','modules']) {
  ...
  const samples=await page.evaluate(()=>{
   ...
   return [...document.querySelectorAll('.masthead-name,.hero-kicker,.masthead-bio,.wire-key,.wire-val,.portrait-note,.path-label,.path-reset,.index-mark,.index-link,.index-cta,.sheet-title,.sheet-tally,.about-text-wrapper p,.project-name,.project-links a,.note-name,.module-btn,.footer-invitation,.foot-sign')].flatMap(e=>{
